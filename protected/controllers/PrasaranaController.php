...
class PrasaranaController extends Controller
{
	...
	public function actionCreate()
	{
		...
		{
			$model->attributes=$_POST['Prasarana'];
			// tambahan
			// $model->lampiran=CUploadedFile::getInstance($model,'lampiran');
			if($model->save()){
				// $nama = $model->lampiran->getName();
				// $model->lampiran->saveAs(Yii::app()->basePath.'/../file/Prasarana/'.$nama);
				$this->redirect(array('view','id'=>$model->id_prasarana));
			}
			// end tambahan
		}
	...
	{
		$dataProvider=new CActiveDataProvider('Prasarana');
	...
	{
		$model=new Prasarana('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Prasarana']))
			$model->attributes=$_GET['Prasarana'];
	 ...
	 * @return Prasarana the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Prasarana::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param Prasarana $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='prasarana-form')
		{
