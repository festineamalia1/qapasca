...
class AktivitasDosenTetapSemesterController extends Controller
{
	/**
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('admin','create','delete','update','index','view','GetData_dosentetap_semester','sentdata','CetakPDFDosentetap_semester'),
	...
	{
		$model=new AktivitasDosenTetapSemester;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['AktivitasDosenTetapSemester']))
		{
			$model->attributes=$_POST['AktivitasDosenTetapSemester'];
			// tambahan
			if($model->save())
				$this->redirect(array('view','id'=>$model->id_aktivitas));
			// end tambahan
		}
	...
	public function actionIndex()
	{
		...
		$dataProvider=new CActiveDataProvider('AktivitasDosenTetapSemester', 
			array(
				'criteria'=>$criteria,
				'pagination'=>array(
			        'pageSize'=>10,
	...
	{
		$model=new AktivitasDosenTetapSemester('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['AktivitasDosenTetapSemester']))
			$model->attributes=$_GET['AktivitasDosenTetapSemester'];
	 ...
	 * @return AktivitasDosenTetapSemester the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=AktivitasDosenTetapSemester::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param AktivitasDosenTetapSemester $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='aktivitas-dosen-tetap-semester-form')
		{
	...
	public function actionGetData_dosentetap_semester(){
		...
		$this->render('v_getdata_dosentetap_semester',$data);
	}
	...
	public function actionSentData(){
		...
		$myHtml = $this->renderPartial('v_dosentetap_semester',$data,true); 
		echo $myHtml;
		Yii::app()->end(); 
		return;	
	}	

	public function actionCetakPDFDosentetap_semester(){
        ...
        if($id_prodi == 1){
	        ...
	        $mPDF1->WriteHTML($this->renderPartial('v_pdf_dosentetap_semester_fakultas', $data, true));
        }else{
        	// footer
        	...
        	# renderPartial (only 'view' of current controller)
	        $mPDF1->WriteHTML($this->renderPartial('v_pdf_dosentetap_semester', $data, true));
        }
