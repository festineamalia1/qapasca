...
class TugasBljDosenController extends Controller
{
	/**
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('admin','create','delete','update','index','view','getdata','sentdata','CetakPDFtugasbelajar'),
	...
	{
		$model=new TugasBljDosen;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['TugasBljDosen']))
		{
			$model->attributes=$_POST['TugasBljDosen'];
			// tambahan
			if($model->save()){
				$this->redirect(array('view','id'=>$model->id_kemampuan));
			}
			// end tambahan
		}
	...
	public function actionIndex()
	{
		...
		$dataProvider=new CActiveDataProvider('TugasBljDosen', 
			array(
				'criteria'=>$criteria,
				'pagination'=>array(
			        'pageSize'=>10,
	...
	{
		$model=new TugasBljDosen('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['TugasBljDosen']))
			$model->attributes=$_GET['TugasBljDosen'];
	 ...
	 * @return TugasBljDosen the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=TugasBljDosen::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param TugasBljDosen $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='tugas-blj-dosen-form')
		{
	...
	public function actionGetData(){
		...
		$this->render('v_getdata_tugasbelajar',$data);
	}
	...
	public function actionSentData(){
		...
		// var_dump($data['data']);
		$myHtml = $this->renderPartial('v_tugasbelajar',$data,true); 
		echo $myHtml;
		Yii::app()->end(); 
		return;	
	}	

	public function actionCetakPDFtugasbelajar(){
        ...
        if($id_prodi == 1){
	        ...
	        $mPDF1->WriteHTML($this->renderPartial('v_pdf_tugasbelajar_fakultas', $data, true));
        }else{
        	// footer
        	...
        	# renderPartial (only 'view' of current controller)
	        $mPDF1->WriteHTML($this->renderPartial('v_pdf_tugasbelajar', $data, true));
        }
