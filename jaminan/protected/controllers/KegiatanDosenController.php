...
class KegiatanDosenController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('admin','delete','update','create','index','view','getdata','sentdata','CetakPDFkegiatandosen'),
	...
	{
		$model=new KegiatanDosen;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['KegiatanDosen']))
		{
			$model->attributes=$_POST['KegiatanDosen'];
			// tambahan

			if($model->save()){
				
				$this->redirect(array('view','id'=>$model->id_kegiatan_dosen));
			}
			// end tambahan
		}
	...
	public function actionIndex()
	{
		...
		$dataProvider=new CActiveDataProvider('KegiatanDosen', 
			array(
				'criteria'=>$criteria,
				'pagination'=>array(
			        'pageSize'=>10,
	...
	{
		$model=new KegiatanDosen('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['KegiatanDosen']))
			$model->attributes=$_GET['KegiatanDosen'];
	 ...
	 * @return KegiatanDosen the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=KegiatanDosen::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param KegiatanDosen $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='kegiatan-dosen-form')
		{
	...
	public function actionGetData(){
		...
		$this->render('v_getdata_kegiatandosen',$data);
	}
	...
	public function actionSentData(){
		...
		// var_dump($data['data']);
		$myHtml = $this->renderPartial('v_kegiatandosen',$data,true); 
		echo $myHtml;
		Yii::app()->end(); 
		return;	
	}	

	public function actionCetakPDFkegiatandosen(){
        ...
        if($id_prodi == 1){
	        ...
	        $mPDF1->WriteHTML($this->renderPartial('v_pdf_kegiatandosen_fakultas', $data, true));
        }else{
        	// footer
        	...
        	# renderPartial (only 'view' of current controller)
	        $mPDF1->WriteHTML($this->renderPartial('v_pdf_kegiatandosen', $data, true));
        }
