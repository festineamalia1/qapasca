...
?>
			...
			if(isset($perpustakaan)){
				...
				foreach ($perpustakaan as $key => $value) {
					...
					<tr>
						...
						?>
						<td style="text-align:center;"><?=(($value['sistem_pengelolaan'] == 'manual') ? '<img style="max-width:15px;" src="'.Yii::app()->baseUrl.'/images/centang2.jpg">':'tidak')?></td>
						<td style="text-align:center;"><?=(($value['sistem_pengelolaan'] == 'kom non jaringan') ? '<img style="max-width:15px;" src="'.Yii::app()->baseUrl.'/images/centang2.jpg">':'tidak')?></td>
						<td style="text-align:center;"><?=(($value['sistem_pengelolaan'] == 'lan') ? '<img style="max-width:15px;" src="'.Yii::app()->baseUrl.'/images/centang2.jpg">':'tidak')?></td>
						<td style="text-align:center;"><?=(($value['sistem_pengelolaan'] == 'wan') ? '<img style="max-width:15px;" src="'.Yii::app()->baseUrl.'/images/centang2.jpg">':'tidak')?></td>
					</tr>
					<?
				}
			}else{
	...
	<?php 
	if(empty($F_rencana_pengembangan_si)){
		?>
		<div class="alert alert-error">
		  Maaf Data yang Anda inginkan masih dalam keadaan kosong. 
		</div>
...
?>
	...
	<br>
	<p>
		6.5.2 Beri tandan <img style="max-width:15px;padding-bottom:5px;" src="<?php echo Yii::app()->baseUrl;?>/images/centang.png"> (centang) pada kolom yang sesuai (hanya satu kolom) dengan aksesibilitas tiap jenis data, dengan mengikuti format tabel berikut :
	</p>
	<table class="table table-bordered kusus">
		<thead>
			<tr>
