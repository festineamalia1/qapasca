...
class Administrasi extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('th_akademik, th_akreditasi', 'required','message'=>'<i><span style="color:red">Kolom {attribute} tidak boleh dikosongkan</span></i>'),
			array('th_akademik, th_akreditasi', 'length', 'max'=>10),
			array('sumber_data, lampiran', 'length', 'max'=>100),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id_administrasi, th_akademik, th_akreditasi, sumber_data, lampiran', 'safe', 'on'=>'search'),
		);
	}
