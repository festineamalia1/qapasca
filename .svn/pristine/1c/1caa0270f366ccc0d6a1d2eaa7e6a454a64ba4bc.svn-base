...
<div class="row-fluid">
	<div class="span12">
		<form class="form-inline" method="post" id="form_dosen_dan_mahasiswa">		    
		    ...
		    <button type="button" class="btn btn-primary getdata_dosen_dan_mahasiswa"><i class="icon-arrow-right icon-white"></i> Proses</button>
		</form>
		...
		<div id="data_mhs_reg" class="data">	
			<?php $this->renderPartial('pages/v_gr_dosen_dan_mahasiswa', array('prodi'=>$prodi,'dosen_tetap'=>$dosen_tetap,'dosen_tidak_tetap'=>$dosen_tidak_tetap,'mahasiswa'=>$mahasiswa)); ?>
		</div>
	</div>
</div>
<hr>
...
$(function(){
  $(".getdata_dosen_dan_mahasiswa").click(function()
  	{
  		var dataserialized = $('#form_dosen_dan_mahasiswa').serialize();
  		$.ajax({
  			type: 'POST',
  			url:"grafik_dosen_dan_mahasiswa",
  			data: dataserialized,
		   	beforeSend:function(){
