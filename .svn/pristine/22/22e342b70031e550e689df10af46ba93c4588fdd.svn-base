...
class CapaianDosenController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('admin','delete','update','create','index','view','getdata_capaiandosen','sentdata_capaiandosen','cetakPDFcapaiandosen'),
	...
	{
		$model=new CapaianDosen;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['CapaianDosen']))
		{
			$model->attributes=$_POST['CapaianDosen'];
			// tambahan
			
			if($model->save()){
				
				$this->redirect(array('view','id'=>$model->id_capaian));
			}
			// end tambahan
		}
	...
	public function actionIndex()
	{
		...
		$dataProvider=new CActiveDataProvider('CapaianDosen', 
			array(
				'criteria'=>$criteria,
				'pagination'=>array(
			        'pageSize'=>10,
	...
	{
		$model=new CapaianDosen('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['CapaianDosen']))
			$model->attributes=$_GET['CapaianDosen'];
	 ...
	 * @return CapaianDosen the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=CapaianDosen::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param CapaianDosen $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='capaian-dosen-form')
		{
	...
	//capaian dosen
	public function actionGetData_capaiandosen(){
		...
		$this->render('v_getdata_capaiandosen',$data);
	}

	public function actionSentData_capaiandosen(){
		...
		if($id_prodi == 1){
		    ...
		    ->select('c.id_prodi, nama_dosen,prestasi,waktu,tingkat,p.jurusan,a.id_administrasi,a.th_akademik,.th_akreditasi')
			...
			$data['data'] = array();
			$data['data'] = Yii::app()->db->createCommand()
		    ->select('c.id_prodi, nama_dosen,prestasi,waktu,tingkat,p.jurusan,a.id_administrasi,a.th_akademik,a.th_akreditasi')
		    ->from('capaian_dosen c')
		    ->join('prodi p', 'c.id_prodi=p.id_prodi')
		    ->join('administrasi a','a.id_administrasi=c.id_administrasi')
			->andWhere('p.id_prodi=:id', array(':id'=>$id_prodi))
		...
		// var_dump($data['data']);
		$myHtml = $this->renderPartial('v_capaiandosen',$data,true); 
		echo $myHtml;
		Yii::app()->end(); 
		return;	
	}	

	public function actionCetakPDFcapaiandosen(){
        ...
        if($id_prodi == 1){
	        ...
	        $mPDF1->WriteHTML($this->renderPartial('v_pdf_capaiandosen_fakultas', $data, true));
        }else{
        	// footer
        	...
        	# renderPartial (only 'view' of current controller)
	        $mPDF1->WriteHTML($this->renderPartial('v_pdf_capaiandosen', $data, true));
        }
