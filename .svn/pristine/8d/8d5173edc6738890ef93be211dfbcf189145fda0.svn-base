...
class HimpunanAlumniController extends Controller
{
	/**
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('admin','create','delete','index','view','getdata_himpunan','sentdata','CetakPDFHimpunanAlumni'),
	...
	{
		$model=new HimpunanAlumni;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['HimpunanAlumni']))
		{
			$model->attributes=$_POST['HimpunanAlumni'];
			// tambahan
			$model->lampiran=CUploadedFile::getInstance($model,'lampiran');
			if($model->save()){
				if($model->lampiran){
					$nama = $model->lampiran->getName();
					$model->lampiran->saveAs(Yii::app()->basePath.'/../file/HimpunanAlumni/'.$nama);
				}
				$this->redirect(array('view','id'=>$model->id_himpunan));
			}
			// end tambahan
		}
	...
	public function actionIndex()
	{
		...
		$dataProvider=new CActiveDataProvider('HimpunanAlumni', 
			array(
				'criteria'=>$criteria,
				'pagination'=>array(
			        'pageSize'=>10,
	...
	{
		$model=new HimpunanAlumni('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['HimpunanAlumni']))
			$model->attributes=$_GET['HimpunanAlumni'];
	 ...
	 * @return HimpunanAlumni the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=HimpunanAlumni::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param HimpunanAlumni $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='himpunan-alumni-form')
		{
	...
	// 	$this->render('v_getdata_himpunan');
	// }
	...
	public function actionCetakPDFHimpunanAlumni(){
